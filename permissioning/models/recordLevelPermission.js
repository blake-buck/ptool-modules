    ...
    function postRecordLevelPermission({tableName,recordId,permissionType,get,update,delete}){
        return new Promise((resolve, reject) => {
            sqlite.get(
                `INSERT INTO recordLevelPermission(tableName, recordId, permissionType, get, update, delete) VALUES($tableName, $recordId, $permissionType, $get, $update, $delete);`,
                {
                    $tableName:tableName, $recordId:recordId, $permissionType:permissionType, $get:get, $update:update, $delete:delete
                },
                (err) => {
                    if(err){
                        return reject(err);
                    ...
                    sqlite.get(
                        ...
                        (err, idData) => {
                            ...
                            return resolve({
                                id:idData['MAX(id)'],
                                tableName,recordId,permissionType,get,update,delete
                            })
                        }
                    )
                }
    ...
    function updateRecordLevelPermissions(recordLevelPermissionDataArray){
        return Promise.all(recordLevelPermissionDataArray.map(({id, tableName, recordId, permissionType, get, update, delete}) => {
            return new Promise((resolve, reject) => {
                sqlite.run(
                    `UPDATE recordLevelPermission SET tableName=$tableName, recordId=$recordId, permissionType=$permissionType, get=$get, update=$update, delete=$delete WHERE id=$id`,
                    {
                        $id: id,
                        $tableName:tableName, $recordId:recordId, $permissionType:permissionType, $get:get, $update:update, $delete:delete
                    },
    ...
    function updateSpecificRecordLevelPermission({id, tableName, recordId, permissionType, get, update, delete}){
        return new Promise((resolve, reject) => {
            sqlite.run(
                `UPDATE recordLevelPermission SET tableName=$tableName, recordId=$recordId, permissionType=$permissionType, get=$get, update=$update, delete=$delete WHERE id=$id`,
                {
